...
from sklearn.metrics import accuracy_score, confusion_matrix, ConfusionMatrixDisplay, classification_report, balanced_accuracy_score
...
model = svm.SVC(class_weight='balanced', kernel='poly')  # change kernel to test each one, default C = 1 and gamma = scale

model.fit(X_train, Y_train)
Y_Predict = model.predict(X_test)

confMatrix = confusion_matrix(Y_test, Y_Predict)
classRep = classification_report(Y_test, Y_Predict)

print('SVM - Poly Kernel') # change kernel title to match
print(f'-Accuracy of Model = {str(accuracy_score(Y_test, Y_Predict))}')
print(f'-Balanced Accuracy of Model = {str(balanced_accuracy_score(Y_test, Y_Predict))}')
