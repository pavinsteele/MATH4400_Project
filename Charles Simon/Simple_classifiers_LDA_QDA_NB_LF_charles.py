...
crossval_lr = cross_val_score(best, X5_sc, Y, cv=kfold).mean()
print(crossval_lr)
crossval_lr = cross_val_score(best, X5_sc, Y, cv=kfold, scoring='balanced_accuracy').mean()
print(crossval_lr)

"""Naive Bayes(smote only") no hyper param"""
#full
NB = GaussianNB()
NB.fit(X_sm, Y_sm)
...
crossval_NB = cross_val_score(NB, X5_sc, Y, cv=kfold).mean()
print(crossval_NB)
crossval_NB = cross_val_score(NB, X5_sc, Y, cv=kfold, scoring='balanced_accuracy').mean()
print(crossval_NB)
