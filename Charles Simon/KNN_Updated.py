...
#gross adjustment for K
k_range = list(range(1, 50,3))

#special pipline for smote so it only applies oversampling to training data
model = Pipeline([
        ('sampling', SMOTE()),
...
#plotBalanced accuracy vs K value
plt.plot(k_range, grid.cv_results_['mean_test_score'])
plt.xlabel('Value of K for KNN')
plt.ylabel('Cross-Validated Accuracy')
plt.show()

#fine adjustment of k
...
"""Full data set balanced accuracy"""
...
#overall better minority class performance for Balanced accuracy but worse accuracy

"""5% alpha reduced dataset res balanced accuarcy"""
...
"""10% alaph reduced dataset balanced accuracy"""
modelCV = Pipeline([
        ('sampling', SMOTE()),
        ('clf', KNeighborsClassifier(n_neighbors=4))
    ])
